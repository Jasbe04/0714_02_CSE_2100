/**
 * @file file_op.cpp
 * @brief Model layer — CopyOperation, MoveOperation, DeleteOperation.
 *
 * Pure business logic; no GTK widget code.
 * This file is identical in behaviour to the original; it has simply been
 * moved into the Model layer with no architectural changes needed.
 */
...
CopyOperation::CopyOperation(const std::string &src, const std::string &dest, const char *customName)
    : m_src(src), m_dest(dest), m_customName(customName) {}
...
{
    // First phase: copy with dialog title "Moving..."
    CopyOperation copyOp(m_src, m_dest, "Moving...");
    copyOp.execute(reporter);

    // Second phase: delete source with dialog title "Cleaning up original..."
    DeleteOperation delOp(m_src, "Cleaning up original...");
    delOp.execute(reporter);
}
// ── DeleteOperation ───────────────────────────────────────────────────────────

DeleteOperation::DeleteOperation(const std::string &path, const char *customName)
    : m_path(path), m_customName(customName) {}
...
void DeleteOperation::execute(IProgressReporter *reporter)
{
    ...
    // 2. Now calculate the size while the window is visible on screen
    long long total = calculateTotalSize(m_path);
    long long deleted = 0;
    if (total == 0)
        total = 1;

    // 3. Do the actual deletion
